...
export class CodeData{
    ...
    getDataForNewDraw(pers = 0, time_le = 1, time_ri = -1){
        if(time_le === 1 && time_ri === -1) return this.getDataForOneDraw(persistence);

        let le_idx = -1, ri_idx = -1, le_min = Infinity, ri_max = -Infinity;
        let flag = true;
        let data_list = [];
        let cnt = 0;
        for(let i=0; i<this.hill_list.length; i++){
            // There may be any other filter condition if necessary
            // 后面那串保证至少有一个能进到循环里面
            if((this.hill_list[i].start_persistence <= pers && this.hill_list[i].end_persistence > pers)||(flag&&i===this.hill_list.length-1)){
                flag = false;
                let left_time = getDataTime(this.hill_list[i].left_idx);
                let peak_time = getDataTime(this.hill_list[i].top_idx);
                let right_time = getDataTime(this.hill_list[i].right_idx);
                if(left_time>=time_ri || right_time<= time_le ) continue;
                let area_time = right_time - left_time;
                let total_time = time_ri -time_le;
                let persistence = this.hill_list[i].end_persistence;
                for(let j=this.hill_list[i].upper; j !== -1; j=this.hill_list[j].upper){
                    persistence = this.hill_list[j].end_persistence;
                }
                ...
                if(left_time<time_le){
                    left_time = time_le;
                    if(right_time<=time_ri){
                        if(peak_time<time_le){
                            if((right_time-peak_time)*pers>(right_time-time_le)*persistence) continue;
                            else {peak_time = time_le;} // 理论上peak_value和min_value都会变，懒得管了
                        }
                    }
                    else{
                        right_time = time_ri;
                        if(peak_time<time_le) peak_time=time_le;
                        else if(peak_time>time_ri) peak_time=time_ri;
                    }
                }
                else{
                    if(right_time>time_ri){
                        right_time = time_ri;
                        if(peak_time>time_ri){
                            if((peak_time-left_time)*pers>(time_ri-left_time)*persistence) continue;
                            else {peak_time = time_ri;} // 理论上peak_value和min_value都会变，懒得管了
                        }
                    }
                }
                let time_width = right_time - left_time;
                ...
                data_list.push( {
                    'right_idx': this.hill_list[i].right_idx,
                    'left': (left_time - time_le)/total_time,
                    'right':(right_time - time_le)/total_time,
                    'peak_time':(peak_time - left_time)/(right_time - left_time),
                    'area': (this.hill_list[i].value_sum/area_time - this.hill_list[i].min_value)/(this.hill_list[i].max_value - this.hill_list[i].min_value),
                    'width': time_width/total_time,
                    'peak_value':(this.hill_list[i].max_value - this.min_range)/this.value_range,
                    'min_value':(this.hill_list[i].min_value - this.min_range)/this.value_range,
                    'persistence': persistence/this.value_range
                });
        ...
        if(le_idx!== -1) data_list[le_idx]['left'] = 0;
        if(ri_idx!== -1) data_list[ri_idx]['right'] = 1;
