        ...
        deleteVisualCodeList(index){
            ...
            for(let i=0; i<this.visual_list.length; i++){
                if(Number(index) === this.visual_list[i]){
                    this.visual_list.splice(i-1, 1)
                    bus.all.get("updateVisualCode").splice(i-1, 1)
                    bus.all.get("updateOrderRule").splice(i-1, 1)
                    bus.all.get("updateDataOrder").splice(i-1, 1)
                    bus.all.get("changeRectOpacity").splice(i-1, 1)
                    bus.all.get("changeLineOpacity").splice(i-1, 1)
                    bus.all.get("clickForLine").splice(i, 1)
                    // bus.all.get("showSimilarityCode").splice(i-1, 1)
                    // bus.all.get("calculateDistance").splice(i-1, 1)
                    bus.all.get("alignOrder").splice(i-1, 1)
                    bus.all.get("synchronizeScroll").splice(i-1, 1)
                    bus.all.get("updatePersistence").splice(i-1, 1)
                    break
                }
            }
            if(this.sort_idx === index){
                this.sort_idx = this.visual_list[0];
                if(!this.independance){
                    bus.emit("alignOrder", [this.sort_idx, 0])
                } 
            }
