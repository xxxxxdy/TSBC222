<template>
    ...
    <div @mouseenter="rollingForDetail" @mouseleave="rollingCancel">
        ...
        <svg fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" class="twotone" @click="()=>{show_barcode = !show_barcode}">
        ...
        <div class="on-canvas" style="display:inline-block; margin-left: 30px; position: relative;" @mousedown="startSelectFunction">
            <div :class="'code-list'+index" v-for="item in codes_message" >
                <div :style="'height:'+onecode_height+'px'">
                    <VisualCode ref="subcode" v-if="show_barcode" :target="index" :mapping="item.idx" :draw_message="item.value" :line_opacity="line_opacity" :rect_opacity="rect_opacity" :show="item.show" :code_name="item.name" :show_name="show_name" :filter="item.filter"/>
                    <TwoTone v-if="!show_barcode" :target="index" :mapping="item.idx" :show="item.show" :code_name="item.name" :lines="item.lines" :show_name="show_name"/>
                </div>
            </div>
        </div>
...
import { getTTData, visual_line } from '../util/twoToneCalculate'
import TwoTone from './TwoTone.vue'
...
export default{
    ...
    components: { VisualCode, TimeMover, TwoTone},
    emits:['delete'],
    data() {
        return {
            identify: 'A',
    ...
    methods: {
        ...
        fastCutTimeRange(msg){
            ...
            let new_x=0, new_x2=CODE_WIDTH;
            if(msg) {
                new_x = msg[0];
                new_x2 = msg[1];
                let time_le = data_field["mins"][1], time_ri = data_field["maxs"][1];
                this.new_time_le = new_x/CODE_WIDTH*(time_ri-time_le)+time_le;
                this.new_time_ri = new_x2/CODE_WIDTH*(time_ri-time_le)+time_le;
            }
            ...
            for(let i=0, len=visualcode_object[this.data_type].length; i<len; i++){
                data_lists[this.index].push(visualcode_object[this.data_type][i].getDataForNewDraw(this.per_val, this.new_time_le, this.new_time_ri));
            }
        ...
        updateDrawMessage(pers = 0, init = false){
            ...
            for(let i=0, len=visualcode_object[this.data_type].length; i<len; i++){
                data_lists[this.index].push(visualcode_object[this.data_type][i].getDataForNewDraw(pers, this.new_time_le, this.new_time_ri));
            }
        ...
        updateMappingChannel(flag = false){ 
            ...
            for(let i=0, len=data_lists[this.index].length; i<len; i++){
                ...
                let st_time = visualcode_object[this.data_type][d_idx].getStartTime();
                let en_time = visualcode_object[this.data_type][d_idx].getEndTime();

                if(st_time<this.new_time_ri && en_time>this.new_time_le){
                    if(st_time<this.new_time_le){
                        st_time = this.new_time_le;
                        // w2 = 0;
                    }
                    if(en_time>this.new_time_ri){
                        en_time = this.new_time_ri;
                    }
                    w2 = (st_time-this.new_time_le)/(this.new_time_ri-this.new_time_le)*CODE_WIDTH;
                    w1 = (en_time-this.new_time_le)/(this.new_time_ri-this.new_time_le)*CODE_WIDTH-w2;
                }
                ...
                draw_value['lines'] = visual_line[this.index][sort_order[tmp][i]];
        ...
        changeDataValue(){
            ...
            this.updateDrawMessage(this.per_val);
            getTTData(this.index, this.data_type);
            this.updateMappingChannel();
    ...
    created() {
        bus.on("updateVisualCode", msg=>{
            this.new_time_le = data_field['mins'][1];
            this.new_time_ri = data_field['maxs'][1];
        ...
        bus.on("alignOrder", msg=>{
            ...
            let tmp = this.sort_index;
            // console.log(this.sort_index, this.index, this.base_sort);
            this.codes_message.sort(function(a, b){
                return re_sort_order[tmp][a.idx] - re_sort_order[tmp][b.idx]
            })
